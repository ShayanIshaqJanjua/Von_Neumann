...
public class CPU {
    ...
    private static void loadToMemory(String f) {
        ...
        File program = new File(f + ".txt"); //opens file
        String[] temp = new String[MainMemory.length];//initialises string array to hold lines of code
        Scanner readLine = new Scanner("Programs/base.txt");//opens the file with the scanner

        int currIndex =0;
        while(readLine.hasNextLine()) {//loops through every line in the program
            System.out.println(readLine.nextLine());
            temp[currIndex] = readLine.nextLine();//adds the line into the string array
            currIndex++;//increments the counter
        }

        for(int i = 0; i < temp.length; i++) {//loops through each line of code
            String[] s = temp[i].split(" ");//splits the code by the space characters
            System.out.println(s[0]);
            if(s[1] == "DAT"){//checks if the current line of code declares a data location
        ...
        for(String l : temp){//loops through every line of code
            String[] s = l.split(" ");//splits the code into its individual tokens
            System.out.println(s[0]);
